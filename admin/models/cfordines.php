...
class wizardModelcfordines extends JModelList {

    //Add this handy array with database fields to search in
    protected $searchInFields = array( 'cf');

//Override construct to allow filtering and ordering on our fields
    public function __construct($config = array()) {
        $config['filter_fields'] = array_merge($this->searchInFields, array('cf'));
    ...
    protected function getListQuery() {
        ...
        // Select some fields
        $query->select('*')
            ->from('#__clienti_ordine_farmacisti as u')
            ->order('id asc')
        ;
